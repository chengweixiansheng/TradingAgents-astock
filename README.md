...
AI 说「买入」，**历史数据上到底靠不靠谱？** 回测功能帮你验证。

```bash
tradingagents backtest 600519 2024-01-01 2024-12-31
tradingagents backtest 300750,002475 2023-01-01 2024-12-31 --style swing
tradingagents backtest 600519 2024-01-01 2024-12-31 --strategy MaCross --params fast=10,slow=30
```
...
| `--style` | 回测风格：`swing`（波段）/ `day`（日内） | `swing` |
| `--strategy` | 策略名：`MaCross` / `RsiReversion` / `BuyAndHold` | `MaCross` |
| `--params` | 策略参数，key=value 逗号分隔 | 策略默认值 |

**内置策略：**

| 策略 | 逻辑 | 默认参数 |
|------|------|----------|
| `MaCross` | 均线交叉：快线上穿买入，下穿卖出 | fast=20, slow=60 |
| `RsiReversion` | RSI 均值回归：超卖买入，超买卖出 | window=14, buy_below=30, sell_above=70 |
| `BuyAndHold` | 买入持有：第一天全仓买入，持有到结束 | 无 |

**输出指标：**

回测完成后自动输出完整绩效报告，包括：总收益、年化收益、最大回撤、夏普比率、卡玛比率、索提诺比率、胜率、盈亏比、交易笔数、基准对比（等权持有）、风险透视（HHI/波动率/平均仓位）等。

**数据源：** A 股取 baostock 前复权日线（与看板同源），港美股取 yfinance。

**A 股交易规则：** T+1、涨跌停（主板±10%/科创创业板±20%/北交所±30%）、最小 100 股整手、佣金万2.5 + 过户费万0.1 + 印花税万5。

**运行卡片：** 每次回测自动生成 JSON + Markdown 格式的运行卡片（`run_card`），记录数据来源、策略参数、指标结果，供追溯和复算。
