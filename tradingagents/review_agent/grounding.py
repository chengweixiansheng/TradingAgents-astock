...
def build_catalog(values: dict, date: str) -> list[dict]:
    ...
    from tradingagents.review.verification import METRICS
    valid_date(date)
    records = []
    def add(record):
...
def _text(value, maximum=900, names=()):
    ...
    from tradingagents.review.llm_errors import LlmConfigError
...
def validate_summary(obj, records):
    from tradingagents.review.schemas import PHASES
...
def generate_grounded(llm, inputs, date, check, progress):
    ...
    from tradingagents.review.prompts import RESEARCH_PACK
    from tradingagents.review.verification import METRICS
    for name in SOURCES:
        getattr(inputs, name)(date)
    catalog = build_catalog(inputs.values, date)
    from tradingagents.review.reflection import get_past_context
