...
from tradingagents.backtest.run import BacktestNotValid, Result, run
from tradingagents.backtest.strategies import BUILTIN
from tradingagents.backtest.stdio_utf8 import force_utf8_stdio
from tradingagents.backtest.loader import LoaderError, price_basis
...
def _catalog() -> dict:
    ...
    """
    from tradingagents.backtest.gate import MARKETS, STYLES
    return {
...
def _result_view(r: Result) -> dict:
    ...
    keep = ("total_return", "annual_return", "max_drawdown", "sharpe", "calmar", "sortino",
            ...
            "net_win_rate", "net_profit_loss_ratio", "net_profit_factor", "net_max_consecutive_loss")
    benchmark_is_self = "benchmark_ticker" not in m
    disclosures = [price_basis(r.plan.market.key),
                   "胜率、盈亏比、盈利因子原字段按平仓毛盈亏计算；net_ 字段逐笔扣除分摊买入与卖出费用。账户总收益与回撤按已扣费净值计算。",
