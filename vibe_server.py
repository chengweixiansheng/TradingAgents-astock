...
from tradingagents.review.paths import data_path
import html
import hmac
import json
import os
...
from tradingagents.review import live_emotion, overseas, preflight, reflection, review_store, trade_calendar
from tradingagents.review.review_store import md_to_html as _md_to_html, strip_prefix as _strip_prefix
from tradingagents.review.config import make_llm
from tradingagents.review.review_graph import build_review_graph
from tradingagents.review.deepdive.graph import run as deepdive_run
from tradingagents.review.roles import ROLES
# initial_state defined below
from tradingagents.review.util import (
    china_now, china_today, is_a_share_closed, is_degraded_report, is_weekend,
    safe_join, strip_model_noise, validate_trade_date,
)
from tradingagents.review.prompts import PACK
...
from tradingagents.market.product_version import PRODUCT_NAME, PRODUCT_VERSION
...
def _merge_vr_routes() -> int:
    ...
    try:
        import tradingagents.market.vr_app as vr_app  # noqa: PLC0415  vr/app.py（不是本仓库的模块）
...
from tradingagents.review.cli_llm import _BINS_ATTR_NAME as _BINS_ATTR  # 单一来源，见那边注释
...
from tradingagents.review_agent.api import Manager as ReviewAgentManager, create_router as review_agent_router
from tradingagents.review_agent.runtime import Runtime as ReviewAgentRuntime
from tradingagents.review_agent.store import Store as ReviewAgentStore
from tradingagents.review_agent.evidence import EvidenceError as ReviewAgentError
...
def _capture_theme_reasons() -> None:
    ...
    try:
        from tradingagents.review import theme_tree
...
def _capture_archive(date: str) -> None:
    ...
    """
    try:
        from tradingagents.review import archive

        r = archive.capture_day(date)
        if not r.get("ok"):
            print(f"WARN: 原始数据归档失败（{r.get('date')}）：{r.get('reason')}")
    except Exception as exc:  # noqa: BLE001
        print(f"WARN: 原始数据归档异常：{type(exc).__name__}: {exc}")
...
def _capture_backtest_corpus(date: str) -> None:
    ...
    """
    try:
        from tradingagents.review.backtest import capture

        r = capture(date)
        if not r.get("ok"):
            print(f"WARN: 回测语料捕获失败（{r.get('date')}）：{r.get('reason')}")
    except Exception as exc:  # noqa: BLE001
        print(f"WARN: 回测语料捕获异常：{type(exc).__name__}: {exc}")
...
def _wk_fresh(cached: dict) -> bool:
    ...
    from tradingagents.review.weekly import LINEAGE_SCHEMA
    ...
    if cached.get("lineage_schema") != LINEAGE_SCHEMA:
        return False
    try:
        from tradingagents.review.weekly import _last_trade_dates
...
def _weekly(force: bool):
    ...
    from tradingagents.review.weekly import build_weekly
...
def api_verify_menu(date: Optional[str] = None):
    """可选指标清单（前端下拉用）。"""
    from tradingagents.review.verification import DIRECTIONS, METRICS
...
def api_verify_items(date: str):
    from tradingagents.review import verification
...
def api_journal_list(limit: int = 200, offset: int = 0):
    from tradingagents.review import journal
...
def api_positions():
    ...
    """
    from tradingagents.review import journal, positions
...
def api_modes():
    """模式卡列表 + 按版本分段的业绩。"""
    from tradingagents.review import modes
...
def api_risk_report():
    from tradingagents.review import journal, risk
...
def api_risk_attribution():
    ...
    from tradingagents.review import attribution, journal
...
def api_risk_excursion():
    ...
    否则每次打开交易日志页都要干等一轮网络。
    """
    from tradingagents.review import excursion, journal
...
def api_at_risk():
    ...
    from tradingagents.review import at_risk, journal
...
def api_get_equity_base():
    from tradingagents.review import at_risk
...
def api_inbox():
    ...
    from tradingagents.review import inbox, journal
...
def api_risk_rules_schema():
    ...
    from tradingagents.review import risk
...
def api_drift():
    ...
    from tradingagents.review import drift
...
def _bt_path(days: int) -> str:
    ...
    from tradingagents.review.backtest import result_path
...
def _bt_load(days: int) -> Optional[dict]:
    ...
    from tradingagents.review.backtest import load_result
...
def _bt_fresh(cached: dict) -> bool:
    ...
    """
    try:
        from tradingagents.review.trade_calendar import latest_session
...
def api_backtest(request: Request, days: int = 60, refresh: int = 0):
    ...
    """
    from tradingagents.review.backtest import run_backtest
    ...
    try:
        ...
        if bt.get("available"):
            try:
                ...
                print(f"WARN: 回测缓存写入失败（每次请求都会重算）：{type(exc).__name__}: {exc}")
...
# ==================== 主线 B：个股深挖 ====================
from tradingagents.review.deepdive.store import serialize as _serialize_dd
...
def _intraday_scheduler() -> None:
    ...
    while True:
        try:
            from tradingagents.review import intraday
            from tradingagents.review.util import china_now, china_today, is_weekend
                ...
                for slot in intraday.SNAPSHOT_SLOTS:
                    ...
                    if key not in done and slot <= hhmm <= _plus_minutes(slot, 6):
                        ...
                        if r.get("ok"):
                            ...
                            print(f"WARN: 盘中快照 {slot} 失败（窗口内会重试）：{r.get('reason')}")
                        break
        except Exception as exc:  # noqa: BLE001  调度线程绝不能死
            print(f"WARN: 盘中调度异常：{type(exc).__name__}: {exc}")
        time.sleep(60)
...
def _start_intraday() -> None:
    ...
    with _intraday_start_lock:          # 检查与置位必须在同一把锁里，否则可能起两个线程
        ...
        try:
            from tradingagents.review import intraday  # noqa: F401  只探在不在
        except ModuleNotFoundError as exc:
            # 走到这里 = intraday 在，但它依赖的东西缺了 —— 真故障，必须出声
            print(f"WARN: 盘中调度未启动：intraday 的依赖缺失（{exc}）")
            return
        except ImportError as exc:
            if exc.name == "duanxian" and "intraday" in str(exc):
                return          # 该版本不含开盘核验，安静跳过
            print(f"WARN: 盘中调度未启动：导入 intraday 失败（{type(exc).__name__}: {exc}）")
            return
...
def _mount_static() -> None:
    ...
    if not os.path.isdir(_DIST):
        print(f"INFO: 未找到前端构建产物（{_DIST}）。请先执行："
    ...
    print(f"OK React 构建产物已挂载（{_DIST}）")
...
if __name__ == "__main__":
    ...
    if _opted_in_clis():
        ...
        print(f"WARN:  VIBE_ALLOW_UNSAFE_CLI 已放开：{', '.join(sorted(_opted_in_clis()))}"
...
def initial_state(trade_date: str) -> dict:
    """复盘管线初始状态（从 vibe-astock main.py 迁入）。"""
    from tradingagents.review import reflection
    return {
        ...
        "leader_report": "",
        "macro_sector_report": "",
        "emotion_metrics": {}, "market_facts": {},
        "tomorrow_focus": "",
        "focus_struct": None,
        "past_context": reflection.get_past_context(end=trade_date),
    }
